...
- 新主線は `receipt-manager -> l0-001-skill-invoke -> l1-001-skill-plan -> response-manager` とする。
...
- `receipt-manager` は prompt 解釈、`l0-001-skill-invoke` は skill 集合確定、`l1-001-skill-plan` は orchestration、`response-manager` は応答整形に専念させる。
...
| skill 要否判断 detail | `l0-001-skill-invoke` |
| 実行順 detail | `l1-001-skill-plan` |
| 応答整形 detail | `response-manager` |
| matrix、例、細かい手順 | 各 skill の `references/` と proposal 文書 |

## 実装済み skill の扱い

- legacy 入口 skill は `l0-001-skill-invoke` へ名称統一し、旧名称は使わない。
- `l1-001-skill-plan` は存続し、selection owner ではなく orchestration owner に固定する。
