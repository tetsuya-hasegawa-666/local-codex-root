...
| --- | --- |
| 不足 skill | ある |
| 大きすぎる skill | `skill-invoker`, `skill-planner`, `document-control-manager`, `plan-gate-manager`, `worklog-runtime-manager` |
...
| 2. 全体構成 | receipt → invoke → planner → execution → response | shared 固定 | `receipt-manager`, `skill-invoker`, `skill-planner`, `response-manager` | 骨格は shared に残す |
| 3. 強制順序 | 実行順の不変条件 | shared 固定 | `skill-planner`, `write-boundary-guard` | ルールは shared、監視は skill |
| 4. receipt-manager 規則 | prompt 完全読取と解釈 | 既存 skill 維持 | `receipt-manager` | 粒度は妥当 |
| 5. skill-invoker 規則 | skill 集合決定、read set 決定 | 分割推奨 | `skill-invoker`, `rule-snapshot-reader`, `authoritative-doc-scope-resolver`, `skill-opportunity-*` | すでに大きい。`read_set` と `opportunity` は実質別責務 |
| 6. skill-planner 規則 | phase、handoff、close 条件 | 分割推奨 | `skill-planner`, `task-scope-splitter`, `close-condition-definer`, `phase-task-orchestrator` | planner 本体を薄くするのがよい |
...
| 10. directory 統制 | category、tmp、tree 編集禁止 | shared + skill | `workspace-structure-manager`, `write-boundary-guard` | 妥当 |
...
| 14. access / safety | write root と外部書込禁止 | shared + skill | `access-safety-manager`, `write-boundary-guard` | shared に残す比率高めでよい |
...
| 18. 参照先一覧 | 上位文書・関連 skill 参照面 | skill 化推奨 | `document-control-manager`, `reference-rewire-operator` | 静的一覧より自動整合の方が重要 |
...
| 次の一手 | next action を 1 件へ収束 | `current-state-syncer`, `skill-planner` |
...
| `runbook-pair-sync-enforcer` | `.md` / `.ipynb` / source inventory 同期 | runbook pair, source tables | sync update, mismatch report | `script-doc-sync-enforcer` より runbook 特化 |

## 再編推奨

| 現行 skill / manager | 問題 | 再編案 |
| --- | --- | --- |
| `skill-invoker` | read set 判定と opportunity loop が混在 | invoke 本体と opportunity 系の境界を明文化 |
| `skill-planner` | close 条件、phase、next action 集約が重い | `current-state-syncer` と `mrl-ledger-maintainer` を分離 |
