...
- 旧 refresh 版の `l0-001-skill-invoke -> l1-001-skill-plan` 主線を、`receipt-manager -> l0-001-skill-invoke -> l1-001-skill-plan -> response-manager` へ再編した時の役割境界を示す。
...
| 統合入口 | `l0-001-skill-invoke` | 必要 skill 集合、読込下限、defer / block 判定 | 実行順設計、応答整形、直接編集 |
| 順序管理 | `l1-001-skill-plan` | 発火、順序、phase、handoff、close 条件 | skill 集合の再選定、事実改変 |
...
| invoke | `l0-001-skill-invoke` |
| orchestration | `l1-001-skill-plan` |
...
- 現行単一入口は `l0-001-skill-invoke` へ名称統一し、責務は `receipt-manager` と `l0-001-skill-invoke` へ分解する。
- 現行 `l1-001-skill-plan` は維持し、実行順と phase 管理に専念させる。
...
3. `kisaragi-skills/kisaragi_pilot_skill_spec.md` で `receipt-manager`、`l0-001-skill-invoke`、`response-manager` を含む pilot skill 群を定義する。
