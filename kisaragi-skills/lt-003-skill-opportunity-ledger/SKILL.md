---
name: lt-003-skill-opportunity-ledger
...
- `lt-002-skill-opportunity-scout` が候補を返した時にだけ、`l0-001-skill-invoke` から起動される。
...
- 下段分類表の更新は行わない。分類は `lt-004-skill-opportunity-architect` の責務とする。

## Core Workflow

1. `lt-002-skill-opportunity-scout` から渡された候補を確認する。
2. 候補台帳の上段へ append-only で追記する。
3. 同じ更新を更新履歴へも追記する。
