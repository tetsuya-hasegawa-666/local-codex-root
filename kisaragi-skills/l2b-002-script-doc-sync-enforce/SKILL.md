---
name: l2b-002-script-doc-sync-enforce
description: `script`、`notebook`、`runbook` の変更を、関連する正本文書更新、test 実行、記録反映まで同一 task で閉じる specialist skill。発火判断は持たず、`l0-001-skill-invoke` と `l1-001-skill-plan` から同期担当として割り当てられた時だけ動く。
---
...
- `l0-001-skill-invoke` が code / notebook / runbook 変更と文書同期が必要だと判断し、`l1-001-skill-plan` が同期担当として割り当てた時だけ動く。
