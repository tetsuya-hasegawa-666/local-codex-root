---
name: l1-002-phase-task-orchestrate
description: `l1-001-skill-plan` から呼ばれた時に、複数 task 開発依頼を phase 構成へ分解し、authoritative context、設計確定、実装、文書同期、検証、closeout を崩さず進める phase specialist。発火判断は持たず、phase 化と完了条件固定だけを担当する。
---
...
- `l1-001-skill-plan` が multi-phase 管理が必要と判断した時だけ呼ばれる。
...
- `l1-001-skill-plan` から呼ばれる phase specialist とする。
- 設計変更や関数責務整理がある時は `l2a-001-design-first-script-build` を `Phase 2` から併用する。
- path / contract / output 名の切替がある時は `l2a-002-reference-rewire-operate` を `Phase 2` から併用する。
- 文書 drift を閉じる時は `l2a-005-documentation-watchkeep` を `Phase 3` 以降で使う。
- BDD / TDD / gate 更新が主題の時は `l2a-003-delivery-plan-keep` を `Phase 2` または `Phase 5` で併用する。
